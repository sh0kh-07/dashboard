...
import { ArrowRight, Lock, MapPin, Search, X, TrendingDown, TrendingUp } from "lucide-react";
import {
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer,
} from "recharts";

// ------------------------------
// QARSHI SHAHRI MAHALLALARI - KAMBAG'AL OILALAR XIZMATLARI (2026)
// ------------------------------
const mahallaNames = [
...
const totalLegalJobsInCity = 3379;
...
const sortedForTable = [...mahallaData].sort((a, b) => {
  ...
  const order = { "Xavf ostida": 0, "O‘rtacha": 1, "Yaxshi": 2 };
...
const chartData = [...mahallaData]
  ...
  .map(m => ({
    name: m.name,
    "Qamrab olingan oila": m.legalJobs,
  }));

const PoorServicesVilDetail = () => {
  const [brand600, red400, yellow400, green400] = useToken("colors", [
    "brand.600", "red.500", "yellow.500", "green.500",
  ]);
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<"all" | "Xavf ostida" | "O‘rtacha" | "Yaxshi">("all");

  const handleDetailClick = (mahallaName: string) => {
    if (mahallaName === "Batosh") {
      navigate("/poor-services/vil/qarshi/batosh");
    }
  };
      ...
      <Flex direction="column" gap={4}>
        <Flex alignItems="baseline" justifyContent="space-between" flexWrap="wrap" gap={4}>
          <Box>
            <Heading as="h1" size="xl" fontWeight="bold" color="gray.800">
              Qarshi shahri - Kambagʻal oilalarni ularning daromadini oshirishga qaratilgan xizmatlar bilan qamrab olish (2026-yil)
            </Heading>
            <Text fontSize="md" color="brand.300" mt={1}>
              Mahallalar bo'yicha maqsadli ko'rsatkichlar tahlili
            </Text>
          </Box>
          <Flex alignItems="center" gap={2} textAlign="right">
            ...
            <Text fontSize="2xl" fontWeight="extrabold" color={brand600}>
              {stats.totalJobs.toLocaleString()} oila
            </Text>
          </Flex>
        </Flex>
        ...
        <SimpleGrid columns={{ base: 1, md: 3 }} spacing={4} my={2}>
          <Stat bg="white" border="1px solid" borderColor="gray.200" boxShadow="sm" p={4} borderRadius="lg">
            ...
            <StatHelpText>Yuqori qamrov</StatHelpText>
          </Stat>
          <Stat bg="white" border="1px solid" borderColor="gray.200" boxShadow="sm" p={4} borderRadius="lg">
          ...
          <Stat bg="white" border="1px solid" borderColor="gray.200" boxShadow="sm" p={4} borderRadius="lg">
            ...
            <StatHelpText>Kam qamrov</StatHelpText>
          </Stat>
        </SimpleGrid>
        ...
        <Flex direction={{ base: "column", md: "row" }} gap={4} mb={6}>
          ...
          <Select width="220px" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as any)} bg="white" borderColor="gray.200" color="gray.800">
            ...
            <option value="Yaxshi">Yaxshi (yashil)</option>
            <option value="O‘rtacha">O‘rtacha (sariq)</option>
            <option value="Xavf ostida">Xavf ostida (qizil)</option>
          </Select>
        </Flex>
          ...
          <Table variant="unstyled">
            <Thead bg="gray.50" color="gray.700">
              <Tr>
                ...
                <Th isNumeric color="gray.700">Qamrab olinadigan oilalar soni</Th>
        ...
        <Box mt={6}>
          <Heading size="lg" mb={4} color="gray.800">Mahallalar kesimida oilalarni qamrab olish</Heading>
          <Box bg="white" border="1px solid" borderColor="gray.200" boxShadow="sm" p={4} borderRadius="xl" overflowX="auto">
            <ResponsiveContainer width="100%" height={Math.max(500, chartData.length * 35)}>
              <BarChart layout="vertical" data={chartData} margin={{ left: 140 }}>
                ...
                <Tooltip />
                <Bar dataKey="Qamrab olingan oila" fill={brand600} />
              </BarChart>
            </ResponsiveContainer>
          </Box>
        </Box>
...
export default PoorServicesVilDetail;
