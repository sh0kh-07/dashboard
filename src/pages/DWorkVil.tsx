...
// ------------------------------
// TUMANLAR UCHUN ISHSIZLIK MA'LUMOTLARI
// ------------------------------
interface DistrictUnemployment {
    name: string;
...
function generateDistrictsData(): DistrictUnemployment[] {
    const districtsRaw = [
    ...
    // Viloyat o'rtacha ishsizlik (jadvaldan): Qashqadaryo 4.6%
    const viloyatStart = 4.6;
    // Maqsad: ishsizlikni 4.0% gacha kamaytirish rejasi
    const viloyatTarget = 4.0;
    const results: DistrictUnemployment[] = [];

    for (const district of districtsRaw) {
        // Yil boshi: viloyat startiga yaqin, biroz farq bilan (±0.4)
        let start = viloyatStart + (Math.random() - 0.5) * 0.8;
        start = roundOne(Math.min(5.2, Math.max(4.0, start)));
        // Maqsad: viloyat targetiga yaqin, har bir tumanning og'irligiga qarab farq
        let target = viloyatTarget + (Math.random() - 0.5) * 0.5;
        target = roundOne(Math.min(4.5, Math.max(3.5, target)));
        if (start <= target) target = roundOne(start - 0.2);
        ...
        if (badDistricts.includes(district.name)) {
            actual = roundOne(Math.min(start, expectedAfter4 + 0.5));
            status = "bad";
        } else if (goodDistricts.includes(district.name)) {
            actual = roundOne(Math.max(target, expectedAfter4 - 0.4));
            status = "good";
        } else {
...
const UnemploymentVil = () => {
    ...
    const handleClick = (name: string) => {
        // Agar Qarshi shahri bosilsa, mahallalar darajasiga o‘tish (keyin qo‘shish mumkin)
        if (name === "Qarshi shahri") navigate("/work/vil/qarshi");
    };
        ...
        <Box minH="100vh">
            ...
            <Box bg="white" border="1px solid" borderColor="gray.200" boxShadow="sm" p={4} borderRadius="lg" mb={6}>
                ...
                <Text fontSize="sm" color="gray.600">
                    ...
                    • <strong>Holat</strong> – yashil (yaxshi), sariq (oʻrtacha), qizil (xavf ostida).
                </Text>
            </Box>
            ...
            <Box bg="white" border="1px solid" borderColor="gray.200" boxShadow="sm" p={4} borderRadius="lg" mt={6}>
                <Flex gap={3} align="center">
                    ...
                    <Text fontWeight="medium">
                        Xulosa: Qizil rangdagi tumanlar (Qamashi, Mirishkor, Dehqonobod, Nishon) hozirgi pasayish sur’atida yil oxirigacha maqsadli ishsizlik darajasiga yeta olmaydi.
                    </Text>
                </Flex>
...
export default UnemploymentVil;