...
const LoansDetail = () => {
    ...
    // --- Только O‘zmilliybank: 1500 млрд сум, распределение по регионам (веса) ---
    const totalBudgetMlrd = 1500; // 1500 млрд сум
    ...
    const handleRegionClick = (regionKey: string) => {
        if (regionKey === "Kashkadarya") {
            navigate("/loans-detail/kashkadarya");
        }
    };
    ...
    return (
        <Box>
            <Flex alignItems="start" justifyContent="space-between" mb={6}>
                <Box>
                    ...
                    <Text fontSize="sm" color="gray.600" mt={1}>
                        Uzoq muddatli va imtiyozli kreditlar, biznesni boshlash va kengaytirish uchun
                    </Text>
                </Box>
                <Box textAlign="right">
            ...
            <Text fontSize="sm" color="gray.600" mb={6}>
                Kredit mablag‘larining viloyatlar kesimida taqsimlanishi. Xarita ranglari kambag‘allik darajasiga qarab belgilanadi.
            </Text>

            {/* Карта (слева) и график (справа) */}
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={6} mb={8}>
                    ...
                    {tooltip.visible && tooltip.data && tooltip.data.regionKey && (
                        ...
                        >
                            ...
                            <Text fontWeight="bold" color="#90CDF4" mt={1}>
                                Kreditlar: {regionData[tooltip.data.regionKey]?.value || 0} mlrd so‘m
                            </Text>
                ...
                <Box bg="white" p={4} borderRadius="xl" boxShadow="sm">
                    <Heading as="h2" size="sm" mb={3} color="gray.700">
                        Viloyatlar kesimida kreditlar (mlrd so‘m)
                    </Heading>
